...
def plot_map():
    ...
    sites = ['Morse Lake', 'Olallie Meadows', 'Paradise', 'Stampede Pass', 'Stevens Pass', 'Swift Creek', 'Waterhole', 'Wells Creek', 'White Pass']
    snotels = snotels[snotels['name'].isin(sites)]
    snotels['alt'] = (snotels['geometry'].z/3.28).astype(int).astype(str) + ' m elevation'
    wa_state = data_manager.load_washington_boundary()

    # plot these points on a folium map
    m = folium.Map([wa_state.centroid.y.loc[0], wa_state.centroid.x.loc[0]], zoom_start=7, tiles="openstreetmap")
